...
**Realms of Myth: Ascension (v1.0.0)** — a full fantasy RPG add-on for Minecraft Bedrock & Education Edition. Choose your race & class, master 15 unique abilities, tame and ride dragons, run Ancient Altar rituals to forge Mythic weapons, and climb the Chronicles of the Realm quest chain — all with a fully custom art, particle and audio suite.

> **What's new in v1.0.0 "Ascension"** — 8-quest Chronicles chain, the Mythic weapon tier, dragon taming & riding, Ancient Altar rituals & class prestige, distance-based difficulty scaling, 14 custom particle effects, 41 WAVs across 45+ sound events, full mob animation sets and complete worn-armor visuals. See the [Changelog](#-changelog).

<p align="center">
  <img src="docs/screenshots/banner.png" alt="Realms of Myth — Ascension (texture showcase)" width="720">
  <br><sub>Banner is a texture showcase generated by the procedural art pipeline — not in-game footage.</sub>
</p>

## 🖼️ Texture Showcases

All images below are generated by the procedural pixel-art pipeline (`texture-sources/gen_promo_aaa.py`) directly from the pack's actual textures. They are texture showcases, not in-game screenshots.

| | |
|---|---|
| ![Items](docs/screenshots/grid_items.png) | ![Armor](docs/screenshots/grid_armor.png) |
| *Items & weapons* | *Armor* |
| ![Entities](docs/screenshots/grid_entities.png) | ![Banner](docs/screenshots/banner.png) |
| *Entities & dragons* | *Title banner* |
...
- **Heavy shader packs** — the 128×128 dragon textures + 41 WAV audio files add some load on weak GPUs
...
│   ├── manifest.json
│   ├── textures/                ← 91 PNG textures (items, blocks, entities, icons)
│   ├── models/                  ← 19 geometry models (entity + block + weapon attachables)
│   ├── animations/              ← Entity animation files
│   ├── sounds/                  ← 41 WAV audio files + sounds.json (45+ sound events)
...
Phase 2: Textures & Models        ████████████████████ 100%  ✅  (91 PNGs, 19 geo models, 41 WAVs)
...
Phase 12: Ascension Program       ████████████████████ 100%  ✅  (armor layers, mob anims, 14 particles, 41 WAVs, quests/Mythic/taming/rituals, FX/SFX wiring)
```

**🔜 Next (post-1.0):** multiplayer stress test, 3D Blockbench models for weapons (currently 2D attached sprites), additional boss encounters.

---

## 📝 Changelog

### ✨ v1.0.0 "Ascension" — Full Release

- **📜 Chronicles of the Realm (quest chain)** — 8 progressive quests: kill quests (whelp, troll, giant, dragon), collect quests (mythril, dragon heart), craft/wear milestones. Rewards include loot, XP levels, title messages and sound stings. Track progress with `!quest`.
- **🗡️ Mythic weapon tier (T5)** — Dawnbreaker (radiant smite AoE), Void Reaver (shadow-step teleport), Stormcaller Hammer (real lightning per hit). Full items, recipes, geo models, attachables, textures and script procs.
- **🐉 Dragon taming & riding** — bring an adult Fire or Frost Dragon below 30% HP, earn its loyalty with Mythril Ingots, saddle it and ride with view-direction steering. Breath attack on demand (6s cooldown). Whelps may imprint as follow/sit pets.
- **🔮 Ancient Altar rituals & class prestige** — sacrifice a Dragon Heart + 4 Mythril Ingots to summon 3 elite guardians; slay them to choose a guaranteed Mythic weapon. Reach level 30 and touch the altar to prestige for permanent stacking max HP + class-colored aura.
- **🌍 Difficulty scaling** — hostile mobs scale HP/damage by distance from spawn (×1.0 / ×1.5 / ×2.25).
- **✨ Custom VFX library** — 14 tuned particle effects (fireball trail, frost nova ring, arcane step, holy light beam, dragon breaths and more) replacing every vanilla placeholder.
- **🔊 Full audio suite** — 41 production-quality WAVs across 45+ sound events: ability SFX, mob voices, UI stings, weapon sounds and dragon boss battle music.
- **🎬 Mob animations & worn armor** — full animation sets for all custom entities; all 28 armor pieces have worn layers + attachables, fully visible in third person.
- **🧰 Quality** — integration integrity checker (`check_integration.py`), all JSON validated, texture-key and lang completeness verified.

### v0.3.0 — Gameplay Expansion (merged into v1.0.0)
